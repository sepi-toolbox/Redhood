...
const C = {
  ...
  threeKind: { id: 'threeKind', kind: 'ofKind', count: 3, score: 'sumAll' },
  fourKind: { id: 'fourKind', kind: 'ofKind', count: 4, score: 'sumAll' },
  ...
  chance: { id: 'chance', kind: 'chance', score: 'sumAll' },
};
...
eq('트리플 [4,4,4,2,1] 전체합', evalCategory(C.threeKind, [4, 4, 4, 2, 1]).base, 15);
eq('포카드 실패 [4,4,4,2,1]', evalCategory(C.fourKind, [4, 4, 4, 2, 1]).valid, false);
eq('에이스 [1,1,2,3,4]', evalCategory(C.ones, [1, 1, 2, 3, 4]).base, 2);
eq('찬스 [6,6,5,4,1]', evalCategory(C.chance, [6, 6, 5, 4, 1]).base, 22);
