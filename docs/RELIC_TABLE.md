...
| **도토리 부적** | 풀하우스 피해 1.8배. 대신 노페어가 봉인된다. | `-` |
...
| **은탄환** | 포카드 피해 1.8배. 대신 최대 HP -10. | `-` |
| **운명의 골무** | 매 턴 리롤 +2. 대신 최대 HP -12. | `-` |
| **늑대달 목걸이** | 🔥벼름 1당 모든 족보 피해 +2. | `whetScaleDamage` amount=2 |
| **할머니의 동화책** | 야찌 피해 2배. 대신 매 턴 자해 2 (방어도 무시). | `-` |
...
| **거머리 반지** | HP를 회복할 때마다 3 더 회복. 대신 방어도를 얻을 수 없다. | `-` |
| **불씨 항아리** | 리롤할 때마다 🔥벼름 +1. 대신 최대 HP -8. | `-` |
