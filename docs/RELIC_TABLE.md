...
| **늑대 가죽** | 적을 처치할 때마다 최대 HP +1 (그만큼 채워진다). | `maxHpOnKill` amount=1 |
| **독사과 조각** | HP가 절반 이하일 때 모든 족보 피해 +5. | `lowHpDamage` ratio=0.5 amount=5 |
| **꿀단지** | 전투에서 승리하면 HP 4 회복 · 최대 HP +4. | `healOnVictory` amount=4 + `maxHp` amount=4 |
...
| **도토리 부적** | 풀하우스 피해 1.8배. 대신 노페어가 봉인된다. | `categoryMult` category="fullHouse" mult=1.8 + `sealCategory` category="chance" |
| **문지기의 빗장** | 턴이 지나도 방어도가 그대로 남는다. | `blockKeep` ratio=1 |
| **네잎클로버** | 보상에서 레어·에픽이 나올 확률 2배. | `luck` mult=2 |
| **은식칼** | 스트레이트 피해 1.6배. | `categoryMult` category="largeStraight" mult=1.6 |
| **은탄환** | 포카드 피해 1.8배. 대신 최대 HP -10. | `categoryMult` category="fourKind" mult=1.8 + `maxHp` amount=-10 |
| **운명의 골무** | 매 턴 리롤 +2. 대신 최대 HP -12. | `extraReroll` amount=2 + `maxHp` amount=-12 |
| **늑대달 목걸이** | 🔥벼름 1당 모든 족보 피해 +2. | `whetScaleDamage` amount=2 |
| **할머니의 동화책** | 야찌 피해 2배. 대신 매 턴 자해 2 (방어도 무시). | `categoryMult` category="yahtzee" mult=2 + `turnSelfDamage` amount=2 |
...
| **거머리 반지** | HP를 회복할 때마다 3 더 회복. 대신 방어도를 얻을 수 없다. | `healBonus` amount=3 + `noBlock` |
| **불씨 항아리** | 리롤할 때마다 🔥벼름 +1. 대신 최대 HP -8. | `whetOnReroll` amount=1 + `maxHp` amount=-8 |
