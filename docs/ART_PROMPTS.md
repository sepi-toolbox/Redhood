...
> **남은 작업만 한 장으로 보려면 `docs/ART_TODO.md`를 보세요.** (구 REMAINING.md 대체) 이 문서는 완료분까지 포함한 전체 기록입니다.
...
26. **한 상태의 덮개와 아이콘은 같은 모티프로 한 번에 뽑는다.** 따로 뽑았더니 독은 덮개가 초록 점액,
    아이콘은 송곳니가 되어 같은 상태로 안 읽혔다. `statuses.json`의 색까지 공유해야 묶인다.
    → `docs/STATUS_ART_PROMPTS.md` 0부의 모티프 사전을 먼저 정하고, 그 다음에 두 프롬프트를 쓴다.

27. **한 쌍을 묶는 첫째 단서는 색이다.** 같은 상태의 덮개와 아이콘은 모티프보다 색이 먼저 맞아야 한다.
    13px 배지에서는 형태가 뭉개져 색만 남기 때문. 프롬프트에 `The dominant color must be {코드}` 를
    명시하고, 메달 안판도 그 색으로 물들인다.
