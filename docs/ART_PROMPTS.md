# REDHOOD 아트 리소스 프롬프트 팩 — v5 (남은 전량)

v5 개정: 몬스터 44종 체계로 확장 반영(누락 25종 전량 수록), **키잉 실패 방지 규칙**과 **구도 규칙** 추가(높은 주사위·트롤 사고에서 배운 것), 유물 26종 묘사 전량 작성, 진행 현황 v0.77 기준 갱신.
...
|---|---|
| 키아트 4종 | ✅ |
| 주사위 6면 시트 13종 | ✅ (높은 주사위 v0.77 재추출) |
...
| 배경 12종 | 🔶 4종 완료 (숲·개울·오두막·양피지) / 8종 대기 — [4] |
| 족보 판 18종 (종이+아이콘 통합) | ⬜ 대기 — `docs/COMBO_PLATE_PROMPTS.md` |
| ~~줄 UI 원형 아이콘 18종~~ | ❌ 폐기 — 족보 판에 통합됨 |
| 유물 26종 | ⬜ 대기 — [7] |
| 앱 아이콘 | ⬜ 대기 — [8] (맨 마지막) |

**우선순위 제안**: 배경 8종 → 족보 판 18종 → 보물상자 3종 → 보스 낙서 9종 → 유물 26종 → 앱 아이콘.
