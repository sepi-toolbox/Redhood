# 유물 아이콘 — **32/32 완료** (v3.66)

> 전부 들어왔다. `docs/relics_done.png` 참고. 32종이 제 그림을 갖고 `assets/relics/` 에 구워져
> `RELIC_ART_READY` 에 올라 있고, 서비스워커 목록에도 등록해서 오프라인에서도 뜬다.
> 유물 줄에 이모지로 뜨는 자리는 이제 없다.
>
> 어두운 편인 셋: 까마귀 깃털 23 · 말라붙은 심장 33 · 붉은 망토 45.
> 앞의 둘은 검은 깃털·검은 심장이라 어쩔 수 없는 값이고 화면에서는 금빛 테두리와 윤곽으로 읽힌다.
>
> 아래 프롬프트는 다시 뽑을 때를 위한 기록으로 남긴다.
