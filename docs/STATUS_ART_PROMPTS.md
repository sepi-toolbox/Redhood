...
이 문서의 본문은 v3.10 기준이다. 그 뒤 상태이상 구조가 바뀌어 아래가 달라졌다.
**밀린 작업 목록과 완성 프롬프트는 `node tools/arttodo.mjs` 가 뽑는다** (docs/ART_TODO_NOW.md).
이 문서는 '어떤 규격·어떤 머리 블록으로 뽑는가'만 정한다 — 생성기가 여기서 머리 블록을 읽어 간다.

## 1. 굳음은 없어졌다 (v4.8)

한때 지속 방해였다가(v3.10) 주사위 상태이상으로 내려왔고(v3.99) 결국 지웠다.
「이 칸에 6이 나오면 0」은 조건이 붙어 화면에서 읽히지 않았고, 기절과 그림도 겹쳤다.
거는 두 행동(옹이 골렘·비명 지르는 돌의 「굳히기」)은 **기절 1칸**으로 바꿨다 —
기절의 모티프가 '돌처럼 굳은 균열'이라 기술명은 그대로 쓴다.
`fx_petrify.png` 는 쓰는 데가 없어져 삭제했다.

## 1-1. 반사와 가시는 지금 그림 그대로 간다 (성권 확정)

`fx_reflect`(반사 — 가시 돋친 방패)와 `fx_holdtax`(가시 — 가시덩굴 감긴 주먹)가
둘 다 가시 모티프라 한 줄에 서면 헷갈릴 수 있다는 지적이 있었으나, **그대로 두기로 했다.**
다시 꺼내지 말 것.

## 2. 사라진 것 · 옮겨간 것

- **문턱(fx_ward) · 상한(fx_cap)** — v3.90 에 제거. 색 `#6b7a5a` `#55606b` 는 비었다.
- **독(status_die_poison)** — v3.95 에 본체 중독으로 나갔다. 덮개는 삭제했다.
- **물기** — 본문 표는 `status_die_lock.png` 로 적혀 있으나 실제 파일은
  덮개 `assets/ui/status_die_bite.png` · 아이콘 `assets/icons/status_lock.png` 다.

## 3. 새로 늘어난 표식 3종 (본체 버프 — 덮개 없음, 아이콘만)

주사위에 붙지 않으므로 덮개가 필요 없다. 색은 `js/main.js` 의 `BUFF_TONE` 이 정본이다.

| 표식 | 색 | 배정 근거 |
|---|---|---|
| 철갑 | `#4a6fa0` | 남은 자리 중 강철 대역에서 가장 먼 곳 (최소 ΔE 24.7 · 마비와 가장 가까움) |
| 행운 | `#cfe07a` | 최소 ΔE 25.9 · 중독과 가장 가까움 |

**v4.7 — 가시는 없어졌다.** 「맞으면 되돌려준다」는 적이 지닌 **반사**와 같은 물건이라
이름·색·그림(`fx_reflect`)을 통째로 물려받았다. 새로 그릴 것이 하나 줄었다.
반대로 지속 방해 `holdTax` 는 원래 이름 **가시**로 돌아갔다 (그림이 가시덩굴 감긴 주먹).

## 4. 색 자리가 거의 찼다

v3.99 에 걸리는 것을 전부 체력바 아래 한 줄로 모으면서, 예전에는 서로 다른 자리에
있어 색을 겹쳐 써도 됐던 것들이 이제 **한 줄에 최대 21개까지 나란히 선다.**
본문 0부의 "색이 첫째" 원칙(최소 ΔE 24.8)을 지킬 자리가 거의 남지 않았다.
다음에 표식을 늘릴 때는 색보다 **형태(실루엣)**로 가르는 쪽을 먼저 검토해야 한다.
