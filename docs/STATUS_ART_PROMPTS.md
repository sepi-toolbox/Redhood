...
## 1. 굳음은 이제 기절색을 물려받지 않는다

본문 B절의 규칙은 "뜻이 같은 것은 색을 물려받는다 — 굳음=기절색"이었다.
그때 굳음은 **그 칸에 기절을 붙이는 지속 방해**였으니 같은 색이 맞았다.
v3.99 에 굳음이 직접 칸에 붙는 상태이상이 되면서 그 전제가 깨졌다 —
이제 기절과 굳음이 한 줄에 나란히 설 수 있다.

| 상태 | 색 | 비고 |
|---|---|---|
| 기절 | `#c9c2b0` | 그대로 |
| 굳음 | `#6b6055` | 새로 배정. 옛 상한(fx_cap `#55606b`) 자리를 쓰지 않는다 |

색만으로는 부족하다 — 둘 다 '돌'이라 형태까지 갈라야 한다.
**기절은 주사위 전체가 갈라진 것**, **굳음은 눈금 하나가 돌혹으로 부푼 것**으로 실루엣을 나눈다.
