...
| 방어 | `intent_defend` | 상태이상 방패를 빌려 쓰는 중 |
...
**「방어」를 방패로 그리면 안 되는 이유**도 같다. 방패 그림은 그 자체가 둥근 판때기라 아무리
「원 없이」라고 적어도 테두리처럼 읽히고, 게다가 상태이상 쪽 방어 표식이 이미 방패라
같은 그림이 두 뜻으로 쓰이게 된다. 그래서 빗장 X 로 간다.
...
**낱개 6종**은 파일만 갈아 끼우면 그대로 반영된다. 휴식만 코드가 한 줄씩 더 필요하다 —
